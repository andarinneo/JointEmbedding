...
class SHAPENETManifoldOnlyLayer(caffe.Layer):
    """
    Load (input image, label image) pairs from the rendered training images.
    ...
    def setup(self, bottom, top):
        ...
        if 'train' in self.split:
            with open(g_syn_images_filelist_train, 'r') as f:
                self.img_indices = f.read().splitlines()
            with open(g_syn_labels_filelist_train, 'r') as f:
                self.label_indices = f.read().splitlines()
            with open(g_syn_images_imageid2shapeid_train, 'r') as f:
                self.imageid2shapeid = f.read().splitlines()
        elif 'val' in self.split:
            with open(g_syn_images_filelist_val, 'r') as f:
                self.img_indices = f.read().splitlines()
            with open(g_syn_labels_filelist_val, 'r') as f:
                self.label_indices = f.read().splitlines()
            with open(g_syn_images_imageid2shapeid_val, 'r') as f:
                self.imageid2shapeid = f.read().splitlines()
    ...
    def reshape(self, bottom, top):
        if self.batch_size <= 1:
            # load image + label image pair
            aux_im = self.load_image(self.idx)
            aux_label = self.load_label(self.idx)

            self.data = self.mask_image(aux_im, aux_label, self.idx)
            self.label = self.load_label(self.idx)
            self.manifold_coord3 = self.load_manifold_coord(self.idx, 3)
            ...
            for i in range(0, self.batch_size, 1):
                ...
                # load image + label image pair
                self.data.append(self.mask_image(aux_im, aux_label, self.idx))
                self.label.append(self.load_label(self.idx))
                self.manifold_coord3.append(self.load_manifold_coord(self.idx, 3))
    ...
    def mask_image(self, image, label, idx):
        """
        Load input image and preprocess for Caffe:
        - cast to float
        - switch channels RGB -> BGR
        ...
        - transpose to channel x height x width order
        """
        # Use label to mask RGB image
        part_id = 3  # Mask part X (3 legs)

        masked_im = image
        delete_indexes = (label != part_id)
        masked_im[0, delete_indexes[0, ...]] = 0
        masked_im[1, delete_indexes[0, ...]] = 0
        masked_im[2, delete_indexes[0, ...]] = 0

        # plt.imshow(masked_im.transpose((1, 2, 0)))
        # plt.show()
