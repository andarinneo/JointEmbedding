...
if not os.path.exists(g_part_image_embedding_training_folder):
    os.makedirs(g_part_image_embedding_training_folder)
    
# Prepare train_val.prototxt
train_val_in = os.path.join(BASE_DIR, 'train_val_'+g_network_architecture_name+'.prototxt.in')
print 'Preparing %s...'%(g_part_image_embedding_train_val_prototxt)
shutil.copy(train_val_in, g_part_image_embedding_train_val_prototxt)
for line in fileinput.input(g_part_image_embedding_train_val_prototxt, inplace=True):
    ...
    # Arreglar!!!!!!!!!!! poner indices de partes
    for i in range(1, g_n_parts+1):
...
print 'Preparing %s...'%(g_part_image_embedding_solver_prototxt)
shutil.copy(solver_in, g_part_image_embedding_solver_prototxt)
for line in fileinput.input(g_part_image_embedding_solver_prototxt, inplace=True):
...
print 'Preparing %s...'%(g_part_image_embedding_command_sh)
shutil.copy(command_in, g_part_image_embedding_command_sh)
for line in fileinput.input(g_part_image_embedding_command_sh, inplace=True):
    ...
    sys.stdout.write(line)
st = os.stat(g_part_image_embedding_command_sh)
os.chmod(g_part_image_embedding_command_sh, st.st_mode | stat.S_IEXEC)
