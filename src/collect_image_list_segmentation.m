function [ image_list ] = collect_image_list_segmentation( folder, shape_list_file )
...
while ischar(line)
    ...
    image_count = image_count + length(dir(fullfile(shape_images_folder, '*003.png')));
...
while ischar(line)
    shape_property = strsplit(line, ' ');
    shape_images_folder = fullfile(folder, shape_property{1}, shape_property{2});
    shape_images = extractfield(dir(fullfile(shape_images_folder, '*003.png')), 'name');
