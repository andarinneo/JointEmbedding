function [image_list, image_count] = collect_image_list_fcn(folder, shape_list_file, output_file, key)
...
while ischar(line)
    ...
    image_count = image_count + length(dir(fullfile(shape_images_folder, key)));
    
    line = fgetl(shape_list_fid);
end
fclose(shape_list_fid);

%% Allocate cells and save txt file
...
while ischar(line)
    shape_property = strsplit(line, ' ');
    shape_images_folder = fullfile(folder, shape_property{1}, shape_property{2});
    shape_images = extractfield(dir(fullfile(shape_images_folder, key)), 'name');
